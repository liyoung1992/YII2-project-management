<div class="header bg-main">
    <div class="logo margin-big-left fadein-top">
        <h1><img src="images/y.jpg" class="radius-circle rotate-hover" height="50" alt="" />数据管理中心</h1>
    </div>
    <div class="head-l" style="float: right;margin-right: 30px" >
<!--        <a class="button button-little bg-green" href="" target="_blank">-->
<!--            <span class="icon-home"></span> 前台首页</a> &nbsp;&nbsp;-->
<!--        <a href="##" class="button button-little bg-blue">-->
<!--            <span class="icon-wrench"></span> 清除缓存</a> &nbsp;&nbsp;-->
        <a class="button button-little bg-red" href="/index.php?r=user/login">
            <span class="icon-power-off"></span> 退出登录</a> </div>
</div>